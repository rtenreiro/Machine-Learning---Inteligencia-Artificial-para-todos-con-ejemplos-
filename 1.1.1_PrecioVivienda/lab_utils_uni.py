...
def plt_house_x(X, y,f_wb=None, ax=None):
    ''' Vivienda parcela con aXis '''
    if not ax:
        fig, ax = plt.subplots(1,1)
    ax.scatter(X, y, marker='x', c='r', label="Valor Actual")

    ax.set_title("Precio viviendas")
    ax.set_ylabel('Precio (en miles de euros)')
    ax.set_xlabel(f'Superficie en m2/100)')
    if f_wb is not None:
        ax.plot(X, f_wb,  c=dlblue, label="Nuestra predición")
    ax.legend()


def mk_cost_lines(x,y,w,b, ax):
    ''' Hace líneas de costes verticales'''
    cstr = "coste = (1/m)*("
    ctot = 0
    label = 'Coste diferencial'
    addedbreak = False
    for p in zip(x,y):
        f_wb_p = w*p[0]+b
        c_p = ((f_wb_p - p[1])**2)/2
...
def plt_intuition(x_train, y_train):
    ...
    def func( w=150):
        ...
        ax[1].scatter(w,cur_cost, s=100, color=dldarkred, zorder= 10, label= f"coste s w={w}")
        ...
        ax[1].set_title("Costes vs. w, (b= 0,1)")
        ax[1].set_ylabel('Costes')
        ax[1].set_xlabel('w')
        ax[1].legend(loc='upper center')
        fig.suptitle(f"Minimizar costes: Coste actual = {cur_cost:0.0f}", fontsize=12)
        plt.show()
...
def plt_stationary(x_train, y_train):
    ...
    cscat  = ax[1].scatter(w0,b, s=100, color=dlblue, zorder= 10, label="coste")
    ...
    ax[1].text(0.5,0.95,"",  bbox=dict(facecolor='white', ec = 'black'), fontsize = 10,
...
class plt_update_onclick:
    ...
    def __call__(self, event):
        if event.inaxes == self.ax[1]:
            ...
            a = self.ax[1].scatter(ws,bs, s=100, color=dlblue, zorder= 10, label="coste con \nactual w,b")
...
def soup_bowl():
    """ Crea figuras y tramas con una proyección 3D"""
...
def plt_divergence(p_hist, J_hist, x_train,y_train):
    ...
    fig.suptitle(f"El coste aumenta cuando la tasa de aprendizaje es demasiado alta")
    #===============
    #  First subplot
    #===============
    ...
    ax.set_title("Coste vs w, b establecido en 0,1")
    ax.set_ylabel('Coste')
    ax.set_xlabel('w')
...
def plt_gradients(x_train,y_train, f_compute_cost, f_compute_gradient):
    ...
    ax[0].set_title("Coste vs w, con gradiente; b establecido en 0,1")
    ax[0].set_ylabel('Coste')
    ax[0].set_xlabel('w')
    ...
    ax[1].set_title('El gradiente se muestra en el gráfico de carcaj')
