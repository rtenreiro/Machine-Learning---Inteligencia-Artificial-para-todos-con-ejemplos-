...
def plot_progress_kMeans(X, centroids, previous_centroids, idx, K, i):
    ...
    plt.title("Interacciones número %d" %i)
...
def plot_kMeans_RGB(X, centroids, idx, K):
    ...
    ax.set_xlabel('R value - Rojizos')
    ax.set_ylabel('G value - Verdes')
    ax.set_zlabel('B value - Azuless')
    ax.w_yaxis.set_pane_color((0., 0., 0., .2))
    ax.set_title("'Colores originales y de sus clusters' centroids")
    plt.show()
