...
def plt_train_test(X_train, y_train, X_test, y_test, x, y_pred, x_ideal, y_ideal, degree):
    ...
    ax.set_title("Bajo rendimiento con los datos del test",fontsize = 12)
    ax.set_xlabel("x")
    ax.set_ylabel("y")

    ax.scatter(X_train, y_train, color = "red",           label="Aprendizaje")
    ax.scatter(X_test, y_test,       color = dlc["dlblue"], label="Test")
    ax.set_xlim(ax.get_xlim())
    ax.set_ylim(ax.get_ylim())
    ax.plot(x, y_pred,  lw=0.5, label=f"predición, grado={degree}")
...
def plt_optimal_degree(X_train, y_train, X_cv, y_cv, x, y_pred, x_ideal, y_ideal, err_train, err_cv, optimal_degree, max_degree):
    ...
    ax[0].set_title("predición vs datos",fontsize = 12)
    ax[0].set_xlabel("x")
    ax[0].set_ylabel("y")

    ax[0].plot(x_ideal, y_ideal, "--", color = "orangered", label="y_ideal", lw=1)
    ax[0].scatter(X_train, y_train, color = "red",           label="entrenamiento")
    ...
    ax[1].set_title("error vs grado",fontsize = 12)
    cpts = list(range(1, max_degree+1))
    ax[1].plot(cpts, err_train[0:], marker='o',label="error entrenamiento", lw=2,  color = dlc["dlblue"])
    ax[1].plot(cpts, err_cv[0:],    marker='o',label="error cv",  lw=2, color = dlc["dlorange"])
    ax[1].set_ylim(*ax[1].get_ylim())
    ax[1].axvline(optimal_degree, lw=1, color = dlc["dlmagenta"])
    ax[1].annotate("grado óptimo", xy=(optimal_degree,80000),xycoords='data',
                xytext=(0.3, 0.8), textcoords='axes fraction', fontsize=10,
                   arrowprops=dict(arrowstyle="->", connectionstyle="arc3", 
                                   color=dlc['dldarkred'], lw=1))
    ax[1].set_xlabel("grado")
    ax[1].set_ylabel("error")
    ax[1].legend()
    fig.suptitle("Busca grado óptimo ",fontsize = 12)
    plt.tight_layout()
...
#   ax[0].plot(x, y_pred[:,:],  lw=0.5, label=[f"$\lambda =${i}" for i in lambda_range])
    ...
    ax[1].set_title("error vs regularización",fontsize = 12)
    ax[1].plot(lambda_range, err_train[:], label="error entrenamiento", color = dlc["dlblue"])
    ax[1].plot(lambda_range, err_cv[:],    label="error cv",    color = dlc["dlorange"])
    ax[1].set_xscale('log')
    ax[1].set_ylim(*ax[1].get_ylim())
    opt_x = lambda_range[optimal_reg_idx]
    ax[1].vlines(opt_x, *ax[1].get_ylim(), color = "black", lw=1)
    ax[1].annotate("óptima lambda", (opt_x,150000), xytext=(-80,10), textcoords="puntos desplazamiento",
                  arrowprops={'arrowstyle':'simple'})
    ax[1].set_xlabel("regularización (lambda)")
    ax[1].set_ylabel("error")
    fig.suptitle("Tuning Regularización",fontsize = 12)
    ax[1].text(0.05,0.44,"Alta\nVarianza",fontsize=12, ha='left',transform=ax[1].transAxes,color = dlc["dlblue"])
    ax[1].text(0.95,0.44,"Alto\nSesgo",    fontsize=12, ha='right',transform=ax[1].transAxes,color = dlc["dlblue"])
...
def plt_tune_m(X_train, y_train, X_cv, y_cv, x, y_pred, err_train, err_cv, m_range, degree):
    ...
    ax[0].scatter(X_train, y_train, color = "red",           s=3, label="entrenamiento", alpha=0.4)
    ...
    ax[0].legend(loc='upper left')
    ax[0].text(0.05,0.5,f"grado = {degree}", fontsize=10, ha='left',transform=ax[0].transAxes,color = dlc["dlblue"])

    ax[1].set_title("error vs número de ejemplos",fontsize = 12)
    ax[1].plot(m_range, err_train[:], label="error entrenamiento", color = dlc["dlblue"])
    ax[1].plot(m_range, err_cv[:],    label="error cv",    color = dlc["dlorange"])
    ax[1].set_xlabel("Número de ejemplos (m)")
    ax[1].set_ylabel("error")
    fig.suptitle("Ajuste por número de ejemplos",fontsize = 12)
    ax[1].text(0.05,0.5,"Alta\nVarianza",        fontsize=12, ha='left',transform=ax[1].transAxes,color = dlc["dlblue"])
    ax[1].text(0.95,0.5,"Buen \nAprendizaje", fontsize=12, ha='right',transform=ax[1].transAxes,color = dlc["dlblue"])
    ax[1].legend()
    plt.tight_layout()
    plt.show()  
...
def plt_train_eq_dist(X_train,y_train,classes, X_cv,   y_cv, centers, std):
    ...
    ax[0].set_title("Entrenamiento, CV Datos")
    for c in css:
    ...
    ax[1].set_title("Ajuste ideal", fontsize=14)
...
def plt_nn(model_predict,X_train,y_train, classes, X_cv, y_cv, suptitle=""):
    ...
    ax[0].set_title("datos entrenamiento", fontsize=14)
    ...
    ax[1].set_title("cross-validación datos", fontsize=14)
...
def plot_iterate(lambdas, models, X_train, y_train, X_cv, y_cv):
    ...
    fig.canvas.header_visible = False
    fig.canvas.footer_visible = False
    ax.set_title("error vs regularización",fontsize = 12)
    ax.plot(lambdas, err_train, marker='o', label="error entrenamiento", color = dlc["dlblue"])
    ax.plot(lambdas, err_cv,    marker='o', label="error cv",    color = dlc["dlorange"])
    ax.set_xscale('log')
    ax.set_ylim(*ax.get_ylim())
    ax.set_xlabel("Regularización (lambda)",fontsize = 14)
    ax.set_ylabel("Error",fontsize = 14)
    ax.legend()
    fig.suptitle("Tuning Regularización",fontsize = 14)
    ax.text(0.05,0.14,"ErrorEntrenamiento\ninferior a CV",fontsize=12, ha='left',transform=ax.transAxes,color = dlc["dlblue"])
    ax.text(0.95,0.14,"Similar\nEntrenamiento, CV",    fontsize=12, ha='right',transform=ax.transAxes,color = dlc["dlblue"])
    plt.show()
...
def plt_compare(X,y, classes, simple, regularized, centers):
  ...
  #plt simple   
    ...
    ax[0].set_title("Modelo Simple", fontsize=14)
  ...
  #plt regularized   
    ...
    ax[1].set_title("Modelo regularizado", fontsize=14)
  ...
  #plt ideal
    ...
    ax[2].set_title("Modelo Ideal", fontsize=14)
